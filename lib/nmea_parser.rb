      ...
      def parse_sentance(raw)
        data, type, line = validate_raw(raw)
      	...
      	case type
      		when "GGA"
      			data[:type]    = type
      			data[:fix_taken_at]				  = line.shift
            data[:latitude]     = line.shift
      			data[:direction_latitude]			= line.shift
            data[:latitude]     = latLngToDecimal(data[:latitude], data[:direction_latitude], true)
            data[:longitude]    = line.shift
      			data[:direction_longitude]			= line.shift
      			data[:longitude]    = latLngToDecimal(data[:longitude], data[:direction_longitude], false)
      			data[:gps_quality]			= line.shift
      			data[:satellites_tracked]			= line.shift.to_i
      			data[:horizontal_dilution_position]				  = line.shift
      			data[:altitude]			= line.shift
      			data[:altitude_unit]			= line.shift
      			data[:geoid_height_unit]		   = line.shift
      			data[:height_geoid_unit] = line.shift
      			data[:last_update_at]		= line.shift
      			data[:reference_station_id]				  = line.shift
          # when "RMC"
          #   data[:last_nmea] = type
